...
public class IceSprite extends IceElementalMob implements RangedAttackMob {
    ...
    public IceSprite(EntityType<? extends Monster> pEntityType, Level pLevel) {
    ...
    public BrainActivityGroup<? extends IceElementalMob> getFightTasks() {
        return BrainActivityGroup.fightTasks(
                ...
                new AnimatableRangedAttack<>(12)
                        .attackRadius(20)
                        .attackInterval(livingEntity -> 90)
                        .whenStarting(livingEntity -> {
                            triggerAnim("Projectile", "shoot");
                            this.level().playSound((Player) null, this.getX(), this.getY(), this.getZ(), SoundEvents.ILLUSIONER_PREPARE_MIRROR, this.getSoundSource(), 1.0F, 1.0F + (this.random.nextFloat() - this.random.nextFloat()) * 0.2F);
                        })
    ...
    public void performRangedAttack(LivingEntity pTarget, float pVelocity) {
        IceSpriteProjectile spriteProjectile = new IceSpriteProjectile(this.level(), this);
